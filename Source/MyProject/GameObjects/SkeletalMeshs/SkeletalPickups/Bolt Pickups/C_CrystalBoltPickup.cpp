...
void AC_CrystalBoltPickup::Interact_Implementation()
{
	OnInteractBasic();

	// If the value of Souls goes over its max value, set it back to it's max value.
	if (PlayerCharacter->Crossbow.CrossbowObject->BoltsArray[2]->GetDefaultObject<AC_Crossbowbolt>()->NumberOfBolts < 
		PlayerCharacter->Crossbow.CrossbowObject->BoltsArray[2]->GetDefaultObject<AC_Crossbowbolt>()->MaxAmountOfBolts)
	{
		...
		PlayerCharacter->UpdateNumberOfBolts(PlayerCharacter->Crossbow.CrossbowObject->BoltsArray[2]->GetDefaultObject<AC_Crossbowbolt>()->NumberOfBolts);
	}
}
